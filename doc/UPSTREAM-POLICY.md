...
| 2026-07-18 | `70357b961f` | Per-company JWT signing keys for multi-tenant isolation (#5864, security). Our `agent-auth-jwt.ts` signed every company's agent token with one master secret — a real cross-tenant forgery risk. Clean drop-in (source + test byte-identical to upstream pre-image), no migration. **Deviation:** restored the 48h TTL default (upstream lowered it to 1h) in a follow-up commit — the token is minted once per run and AgentDash runs exceed 1h. | `tsc --noEmit` clean; `agent-auth-jwt.test.ts` 11/11. | yt@d4d.group |
| 2026-07-18 | `242a2c2f2b` | CLI `worktree init --force` no longer wipes repo `worktrees/` (#6240). We had the exact `rmSync(paths.repoConfigDir, {recursive})` data-loss path and we use worktrees. | `tsc --noEmit` clean; `cli/.../worktree.test.ts` 35/35. | yt@d4d.group |
| 2026-07-18 | `e93d78b46c` | Harden live-events WS upgrade sockets (#8383). Prevents a process-fatal EPIPE crash when a client disconnects mid-upgrade; `live-events-ws.ts` byte-identical to pre-image. | `tsc --noEmit` clean; `live-events-ws.test.ts` 3/3. | yt@d4d.group |
| 2026-07-18 | `5d315ab778` | Defer same-issue `forceFreshSession` wakes into follow-up runs (#4080). Bug fix in the inherited heartbeat scheduler (`forceFreshSession` is live locally). Source auto-merged; test import conflict resolved by keeping only `shouldDeferFollowupWakeForSameIssue` and dropping `stripHostWorkspaceProvisionForLowTrustSandbox` (a symbol from an upstream commit we don't have, unused by this test). | `tsc --noEmit` clean; `heartbeat-workspace-session.test.ts` 42/42. | yt@d4d.group |
| 2026-09-24 | `1e8ede4e1` | `runChildProcess` escalates to SIGKILL on real process liveness (`exitCode`/`signalCode` still null) instead of `!child.killed` (#8598). `child.killed` turns true as soon as SIGTERM is sent, so on the direct-child fallback path a child that ignored SIGTERM was never force-killed and outlived its deadline. Taken with OBS-5 (#698, stuck vs quiet), the same problem area. The 6-line core change in `server-utils.ts` applied cleanly; the only conflict was the test file's import list (upstream imports SSH/workspace-env helpers we don't have), resolved by adding `signalRunningProcess` to our list. | `packages/adapter-utils/src/server-utils.test.ts` 26/26, including upstream's two new regression tests; `pnpm -r typecheck` clean. | yt@d4d.group |
