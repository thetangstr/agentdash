...
import { AgentDashApiClient, ApiConnectionError, ApiRequestError, PaperclipApiClient } from "../client/http.js";

describe("AgentDashApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new AgentDashApiClient({
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new AgentDashApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("keeps the PaperclipApiClient alias for compatibility", () => {
    expect(PaperclipApiClient).toBe(AgentDashApiClient);
  });
});
