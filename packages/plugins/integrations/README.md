...
Full worker implementations (the plugin code that runs inside the AgentDash runtime) are coming in a future phase.

## Installing a Plugin

1. Copy the plugin directory into your AgentDash plugin registry or reference it from your project configuration.
...
To build a new integration plugin, create a new directory here with a `manifest.json` following the same pattern. The manifest schema is defined by `PaperclipPluginManifestV1` in `@agentdash/shared`.
