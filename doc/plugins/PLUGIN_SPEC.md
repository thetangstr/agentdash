# AgentDash Plugin System Specification
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main AgentDash app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
...
AgentDash plugin design is based on the following assumptions:
...
An installable instance-wide extension package loaded through the AgentDash plugin runtime.
...
Plugins live under the AgentDash instance directory.
...
- `pnpm agentdash plugin list`
- `pnpm agentdash plugin install <package[@version]>`
- `pnpm agentdash plugin uninstall <plugin-id>`
- `pnpm agentdash plugin upgrade <plugin-id> [version]`
- `pnpm agentdash plugin doctor <plugin-id>`
...
- AgentDash server starts one worker process per installed plugin
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@agentdash/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@agentdash/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@agentdash/plugin-sdk/ui`)
...
- Plugin bundles must not import from host internals. They may only import from `@agentdash/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@agentdash/plugin-sdk/ui`
...
The `@agentdash/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
6. The operator may force-purge immediately via CLI: `pnpm agentdash plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the AgentDash server. This is a normative requirement, not optional.
...
### 27.1 `@agentdash/plugin-test-harness`
...
```ts
import { createTestHarness } from "@agentdash/plugin-test-harness";
...
For developing a plugin against a running AgentDash instance:

- The operator installs the plugin from a local path: `pnpm agentdash plugin install ./path/to/plugin`
...
- `@agentdash/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@agentdash/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@agentdash/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@agentdash/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@agentdash/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@agentdash/plugin-sdk/ui`
...
That is the complete target design for the AgentDash plugin system.
