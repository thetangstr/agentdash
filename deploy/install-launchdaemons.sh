...
if [[ "${1:-}" == "--rollback" ]]; then
  for f in "$STAGED"/com.agentdash.*.plist; do
    ...
    launchctl bootstrap "gui/$UID_YANG" "/Users/yang/Library/LaunchAgents/$label.plist" || true
...
for f in "$STAGED"/com.agentdash.*.plist; do
  ...
  launchctl bootout "gui/$UID_YANG" "/Users/yang/Library/LaunchAgents/$label.plist" 2>/dev/null || true
...
# The agents' plists stay in ~/Library/LaunchAgents as inert files for
# rollback; RunAtLoad only fires when bootstrapped, and we booted them out.
# Delete them once the reboot test passes.
