...
usage() {
  cat <<'EOF'
Usage:
  ./scripts/release.sh <canary|stable> [--date YYYY-MM-DD] [--dry-run] [--skip-verify] [--skip-npm] [--print-version] [--allow-canary-latest]

Examples:
  ./scripts/release.sh canary --skip-npm
  ./scripts/release.sh canary --date 2026-03-17 --skip-npm --dry-run
  ./scripts/release.sh stable --skip-npm
  ./scripts/release.sh stable --date 2026-03-17 --skip-npm --dry-run
  ./scripts/release.sh stable --date 2026-03-18 --skip-npm --print-version
...
Notes:
  ...
  - AgentDash canary application releases create the git tag
    canary/vYYYY.MDD.P-canary.N and pass --skip-npm.
  - AgentDash stable application releases create git tag vYYYY.MDD.P and pass
    --skip-npm; package publication is handled separately.
  - Stable release notes must already exist at releases/vYYYY.MDD.P.md.
  - AgentDash application releases pass --skip-npm. Owned npm packages use
    their package-specific trusted-publishing workflows and version lineage.
...
if [ "$skip_npm" = false ]; then
  ...
  [ -n "$PUBLIC_PACKAGE_INFO" ] || release_fail "no owned public packages were found in the workspace."
  for package_name in "${PUBLIC_PACKAGE_NAMES[@]}"; do
    if [ "$package_name" = "agentdash-connect" ]; then
      release_fail "agentdash-connect uses .github/workflows/publish-connect.yml and its 0.1.x lineage; application CalVer releases must pass --skip-npm."
    fi
  done
fi

if [ "$skip_npm" = true ]; then
  TARGET_STABLE_VERSION="$(next_stable_version "$RELEASE_DATE")"
else
...
if [ "$channel" = "canary" ]; then
  require_on_master_branch "$dry_run"
  if [ "$skip_npm" = true ]; then
...
if [ "$skip_npm" = true ]; then
  release_info ""
  release_info "==> Step 3/7: Skipping npm package version rewrites (--skip-npm)..."
  release_info ""
  release_info "==> Step 4/7: Skipping npm package bundle (--skip-npm)..."
  release_info ""
  release_info "==> Step 5/7: Skipping npm publication (--skip-npm)..."
  if [ "$dry_run" = true ]; then
    release_info "  [dry-run] Would create git tag $tag_name on $CURRENT_SHA"
  fi
  release_info ""
  release_info "==> Step 6/7: Skipping npm registry verification (--skip-npm)..."
else
  release_info ""
...
if [ "$dry_run" = true ]; then
  ...
  if [ "$channel" = "canary" ]; then
    release_info "Created application canary ${TARGET_PUBLISH_VERSION}; npm publication was skipped."
    release_info "Next step: git push ${PUBLISH_REMOTE} refs/tags/${tag_name}"
  else
    release_info "Created application stable ${TARGET_PUBLISH_VERSION}; npm publication was skipped."
