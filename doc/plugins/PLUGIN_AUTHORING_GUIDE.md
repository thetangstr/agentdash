...
This guide describes the current, implemented way to create a AgentDash plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main AgentDash app.
...
```bash
pnpm --filter @agentdash/create-paperclip-plugin build
...
Inside this monorepo, the scaffold uses `workspace:*` for `@agentdash/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@agentdash/plugin-sdk` from the local Paperclip checkout into a `.paperclip-sdk/` tarball so you can build and test a plugin without publishing anything to npm first.
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@agentdash/plugin-sdk/ui`
