...
|-------|-------|---------|------------|
| Intake -> Locally-Tested (or Staging-Tested) | `/workon` | Human runs `/workon AD-XXX` | `Locally-Tested` or `Staging-Tested` label set |
...
## Quick Start: `/workon AD-XXX`

The **single entry point** for all development in a workspace:

```bash
/workon AD-123   # Drives issue from intake through Locally-Tested (or Staging-Tested)
```
...
```
+-------------------------------------------------------------+
| /workon AD-XXX (continuous, no human needed)    |
+-------------------------------------------------------------+
|                                                               |
...
/builder AD-5   # Work on specific issue
```
...
```bash
/tester AD-5        # Test specific issue
```
...
**Reference:** See `doc/maw/EPIC_REGISTRY.md` for the full epic/CUJ registry template.
...
| Task | Command |
|------|---------|
| **Start any issue** | `/workon AD-XXX` |
| Skip to Builder | `/builder AD-XXX` |
| Test a PR | `/tester AD-XXX` |
