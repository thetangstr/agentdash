/**
 * esbuild configuration for building the agentdash CLI for npm.
 *
 * Bundles all workspace packages (@agentdash/*) into a single file.
...
// to npm and resolved at runtime (e.g. @agentdash/server uses dynamic import).
...
for (const p of workspacePaths) {
  ...
  for (const name of Object.keys(pkg.dependencies || {})) {
    if (externalWorkspacePackages.has(name)) {
      externals.add(name);
    } else if (!name.startsWith("@agentdash/")) {
      externals.add(name);
    }
  }
