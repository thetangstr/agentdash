...
import { companies, companyMemberships, projects } from "@paperclipai/db";
import { EVALUATION_REVIEW_PROJECT_NAME } from "@paperclipai/shared";
 ...
 * of every company and bring its review items up to date. Deterministic, no
 * model call; the evaluator agent is not involved. Exceptions with no
 * accountable owner route to the company's first active administrator. A
 * company or milestone that fails is logged and skipped; the loop never stops.
 */
...
export function evaluationSnapshotCadence(db: Db, opts: SnapshotCadenceOptions = {}) {
  ...
  async function firstAdministrator(companyId: string): Promise<string | null> {
    ...
    return admins[0] ?? rows.map((r) => r.principalId).sort()[0] ?? null;
  }

  return {
    /** One pass over every company's open projects. */
    async run(): Promise<SnapshotCadenceResult> {
      ...
      for (const { id: companyId } of companyRows) {
        ...
        const fallback = opts.reviewItems === false ? null : await firstAdministrator(companyId);
