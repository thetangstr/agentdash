import type { ExecOsAuditView } from "./execos-audit";
...
export function projectExecOsVoiceTurnAudit(input: {
  executionAudit: ExecOsAuditView | null;
  comments: Array<{ id: string; body: string }>;
}): ExecOsVoiceTurnAuditView | null {
  if (!input.executionAudit) return null;
  for (const comment of input.comments) {
    const audit = parseVoiceAuditComment(comment.body);
    if (!audit) continue;
    if (!matchesExecutionAudit(audit, input.executionAudit)) return null;
    return {
...
function matchesExecutionAudit(audit: ExecOsVoiceTurnAuditComment, executionAudit: ExecOsAuditView): boolean {
  return audit.requestId === executionAudit.requestId
    && audit.correlationId === executionAudit.correlationId
    && audit.terminalStatus === executionAudit.status;
}
