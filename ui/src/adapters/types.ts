...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@agentdash/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@agentdash/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@agentdash/adapter-utils").TranscriptEntry[];
