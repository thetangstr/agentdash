...
// authorized cross-assignee review close, unauthorized and out-of-scope moves, and moves with missing authority evidence.
...
describe("P6 authority — transitions by a non-assignee", () => {
  it("authorized: the reviewer closing in_review→done after its own passed verdict is the sanctioned step (6c)", () => {
    const card = score([...roster(), ...item({ id: I1, started: 1, review: 3 }), verdict(I1, 4, R), move(I1, 5, R, "in_review", "done")]);
    expect(p6(card, R).detail.rules).toEqual({});
    expect(p6(card, R).detail.authorized).toEqual({ verdictClose: 1 });
    expect(p6(card, R).value).toBe(0);
    expect(e3On(card, I1)).toHaveLength(0);
    expect(p6(card, R).notes).toContain("1 close by the reviewer after its own passed verdict: the sanctioned review step");
  });
  ...
  it("out of scope: a passed verdict on a different item does not license the close", () => {
    const card = score([...roster(), ...item({ id: I1, started: 1, review: 3 }), ...item({ id: I2, started: 1, review: 3 }), verdict(I2, 4, R), move(I1, 5, R, "in_review", "done")]);
    expect(p6(card, R).detail.rules).toEqual({ transition_not_assigned: 1 });
    expect(p6(card, R).detail.authorized).toEqual({ verdictClose: 0 });
    expect(e3On(card, I1)).toHaveLength(1);
  });

  it("out of scope: a failed verdict, or a move that is not in_review→done, is not the sanctioned close", () => {
    ...
    const reopen = score([...roster(), ...item({ id: I1, started: 1, review: 3, done: 5 }), verdict(I1, 4, R), move(I1, 6, R, "done", "in_review")]);
    // the close at 5 is sanctioned (verdict at 4); the reopen at 6 is judged on its own and stays a detection
    expect(p6(reopen, R).detail.authorized).toEqual({ verdictClose: 1 });
    expect(p6(reopen, R).detail.rules).toEqual({ transition_not_assigned: 1 });
  });
  ...
  it("a reopen done→in_review by a non-assignee with no verdict stays a detection; the assignee's own moves never count", () => {
