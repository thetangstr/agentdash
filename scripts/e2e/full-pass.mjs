...
// --------------------------------- 11. a file an agent wrote gets an openable URL
section("11. A file the agent wrote becomes a URL a person can open");

// This section touches the instance's filesystem, which the rest of the suite does
// not. That is unavoidable: the whole point is that the agent writes a real file on
// the server's disk and gets back something a reader on another machine can use.
// Skipped rather than failed when the workspace root is not known, so the suite
// still runs against an instance whose disk we cannot see.
const workspaceRoot = process.env.E2E_WORKSPACE_ROOT;
if (!workspaceRoot) {
  console.log("  – skipped (set E2E_WORKSPACE_ROOT to the instance's workspaces/ dir)");
} else {
  const { mkdir, writeFile } = await import("node:fs/promises");
  const { join } = await import("node:path");

  const fileIssue = await call("POST", `/api/companies/${companyId}/issues`, {
    title: "E2E workspace attachment",
    description: "Created by the e2e pass.",
  });
  ok("an issue is created to attach to", [200, 201].includes(fileIssue.status), `status=${fileIssue.status}`);
  const fileIssueId = fileIssue.body?.id;

  // Write a file where the agent would have written it.
  const agentWorkspace = join(workspaceRoot, personalId);
  const expected = `# Q3 report\nGenerated by the e2e pass at stamp ${stamp}.\n`;
  await mkdir(join(agentWorkspace, "reports"), { recursive: true });
  await writeFile(join(agentWorkspace, "reports", "q3.md"), expected, "utf8");

  const attached = await call(
    "POST",
    `/api/companies/${companyId}/issues/${fileIssueId}/attachments/from-workspace`,
    { path: "reports/q3.md" },
    { bearer: personalKey, noCookie: true },
  );
  ok("the agent can attach it", attached.status === 201, `status=${attached.status} ${JSON.stringify(attached.body).slice(0, 160)}`);
  ok("and it is named after the file", attached.body?.originalFilename === "q3.md", attached.body?.originalFilename);
  ok(
    "with a content type from the extension, not the caller",
    attached.body?.contentType === "text/markdown",
    attached.body?.contentType,
  );

  // The point of the whole feature: an absolute URL on the advertised address.
  const advertised = health.body?.publicBaseUrl;
  if (advertised) {
    ok(
      "the url is absolute, on the address the instance advertises",
      attached.body?.url === `${advertised}${attached.body?.contentPath}`,
      JSON.stringify({ url: attached.body?.url, advertised }),
    );
  } else {
    ok("no url is invented when the instance advertises none", attached.body?.url === null, `${attached.body?.url}`);
  }

  // And the bytes actually come back.
  const fetched = await call("GET", attached.body?.contentPath ?? "", undefined, {
    bearer: personalKey,
    noCookie: true,
  });
  ok("fetching the url returns the file's bytes", fetched.text === expected, JSON.stringify(fetched.text ?? "").slice(0, 120));

  // The refusals. These are the reason the endpoint is safe to expose at all.
  ...
  ok("traversal out of the workspace is refused", escape.status === 400, `status=${escape.status}`);
  ...
  ok("an absolute path is refused", absolute.status === 400, `status=${absolute.status}`);

  const missing = await call(
    "POST",
    `/api/companies/${companyId}/issues/${fileIssueId}/attachments/from-workspace`,
    { path: "reports/not-there.md" },
    { bearer: personalKey, noCookie: true },
  );
  ok("a missing file is 404, distinguishable from a refusal", missing.status === 404, `status=${missing.status}`);
  ...
  ok("a signed-in human is refused (they have the file locally)", asHuman.status === 403, `status=${asHuman.status}`);
}
