# `@agentdash/plugin-sdk`

Official TypeScript SDK for AgentDash plugin authors.

- **Worker SDK:** `@agentdash/plugin-sdk` — `definePlugin`, context, lifecycle
- **UI SDK:** `@agentdash/plugin-sdk/ui` — React hooks and slot props
- **Testing:** `@agentdash/plugin-sdk/testing` — in-memory host harness
- **Bundlers:** `@agentdash/plugin-sdk/bundlers` — esbuild/rollup presets
- **Dev server:** `@agentdash/plugin-sdk/dev-server` — static UI server + SSE reload
...
| Import | Purpose |
|--------|--------|
| `@agentdash/plugin-sdk` | Worker entry: `definePlugin`, `runWorker`, context types, protocol helpers |
| `@agentdash/plugin-sdk/ui` | UI entry: `usePluginData`, `usePluginAction`, `usePluginStream`, `useHostContext`, slot prop types |
| `@agentdash/plugin-sdk/ui/hooks` | Hooks only |
| `@agentdash/plugin-sdk/ui/types` | UI types and slot prop interfaces |
| `@agentdash/plugin-sdk/testing` | `createTestHarness` for unit/integration tests |
| `@agentdash/plugin-sdk/bundlers` | `createPluginBundlerPresets` for worker/manifest/ui builds |
| `@agentdash/plugin-sdk/dev-server` | `startPluginDevServer`, `getUiBuildSnapshot` |
| `@agentdash/plugin-sdk/protocol` | JSON-RPC protocol types and helpers (advanced) |
| `@agentdash/plugin-sdk/types` | Worker context and API types (advanced) |
...
- Plugin UI bundles run as same-origin JavaScript inside the main AgentDash app. They can call ordinary Paperclip HTTP APIs with the board session, so manifest capabilities are not a frontend sandbox.
...
- For deployed plugins, publish an npm package and install that package into the AgentDash instance at runtime.
...
```ts
import { definePlugin, runWorker } from "@agentdash/plugin-sdk";
...
**Entity types** (for `entityTypes` on slots): `project` \| `issue` \| `agent` \| `goal` \| `run` \| `comment`. Full list: import `PLUGIN_UI_SLOT_TYPES` and `PLUGIN_UI_SLOT_ENTITY_TYPES` from `@agentdash/plugin-sdk`.
...
Full list in code: import `PLUGIN_CAPABILITIES` from `@agentdash/plugin-sdk`.

## UI quick start

```tsx
import { usePluginData, usePluginAction } from "@agentdash/plugin-sdk/ui";
...
```tsx
import { usePluginData } from "@agentdash/plugin-sdk/ui";
...
import { usePluginAction, type PluginBridgeError } from "@agentdash/plugin-sdk/ui";
...
```tsx
import { useHostContext, usePluginData } from "@agentdash/plugin-sdk/ui";
import type { PluginDetailTabProps } from "@agentdash/plugin-sdk/ui";
...
```tsx
import { usePluginStream } from "@agentdash/plugin-sdk/ui";
...
Each slot type receives a typed props object with `context: PluginHostContext`. Import from `@agentdash/plugin-sdk/ui`.
...
```tsx
import type { PluginProjectSidebarItemProps } from "@agentdash/plugin-sdk/ui";
...
```tsx
import { useState } from "react";
import { usePluginAction, usePluginStream } from "@agentdash/plugin-sdk/ui";
...
```ts
import { createTestHarness } from "@agentdash/plugin-sdk/testing";
...
```ts
import { createPluginBundlerPresets } from "@agentdash/plugin-sdk/bundlers";
...
```ts
import { startPluginDevServer } from "@agentdash/plugin-sdk/dev-server";
