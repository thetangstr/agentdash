...
- `agentdash run --bind <loopback|lan|tailnet>` passes a quickstart bind preset into first-run onboarding when config is missing
- runtime can override mode with `PAPERCLIP_DEPLOYMENT_MODE`
- `agentdash run` and `agentdash doctor` still do not expose a direct low-level `--mode` flag
...
```sh
pnpm agentdash env-lab up
pnpm agentdash env-lab doctor
pnpm agentdash env-lab status --json
pnpm agentdash env-lab down
```
