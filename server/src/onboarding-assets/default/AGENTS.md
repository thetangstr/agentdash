...
- **A reviewer is a hire, and hires wait on a human.** When the queue outgrows the reviewers the system files a `hire_agent` approval for a new CoS Reviewer; until a person approves it, the reviewer cannot run and the Issue legitimately waits. Do not treat that wait as a stuck state to work around — name the pending hire in your comment and escalate to a human rather than closing the Issue yourself or writing the verdict you are owed.
...
When you create, hire, update or import an agent, or set `assigneeAdapterOverrides` on an issue, leave the host-execution fields out of `adapterConfig`. These are `command`, `args`, `extraArgs`, `env` and `cwd`, plus any key ending in `Command`, `Args`, `Env`, `Cwd`, `Dir`, `Home` or `Path` (for example `instructionsFilePath` and `instructionsRootPath`). They decide what runs on the host or which host files the server reads and writes, so only an instance admin may set or change them. An agent request that includes one gets 403 `Instance admin access required`. Retrying with a different spelling will not help.

These stay open to everyone: empty values (the server default), the adapter's default command, values unchanged from what is already stored, and, for Hermes (`hermes_local`), `extraArgs` (an array of non-empty tokens) made only of `-p <profile>` or `--profile <profile>`, `--reasoning-effort <low|medium|high>`, `--max-turns <n>`, `--checkpoints`, and `-v` or `--verbose`. On a box with managed Hermes profiles (every hosted box), each agent runs in its own profile, and `-p` may only name a profile provisioned for an agent of your company (`agentdash-<agent id without dashes>`); any other profile gets 403, and at run time a `-p` that is not the agent's own is ignored. A `*Path` value is also open when it points inside your company's instructions directory under the instance home (the agent's managed bundle or `companies/<companyId>/shared-instructions/`). If a hire needs a custom binary, environment or working directory, file the hire without it and ask a human instance admin in the issue or approval thread.
